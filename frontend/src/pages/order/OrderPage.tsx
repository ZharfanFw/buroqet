...
export default function OrderPage() {
  ...
  const fetchOrders = async () => {
    ...
    try {
      ...
      console.error(err);
      setError('Gagal memuat data order. Pastikan backend server berjalan di port 8082.');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleCreateOrder = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.error(err);
      setActionError(err.response?.data?.error || 'Gagal membuat order. Cek kelengkapan data.');
    } finally {
